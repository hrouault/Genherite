...
:cste(c),phosphosite(NULL)
{
   ...
   meanc=frand2();
   dirinit=(int)(2*frand());
   if (dirinit==0) dirinit=-1;
}

Protein::~Protein()
{
   if (phosphosite) delete phosphosite;
   vector<Composant *>::iterator itercomp;

   for (itercomp=composants.begin();itercomp!=composants.end();itercomp++){
	 delete *itercomp;
   }
}
...
Reaction::Reaction(Espece *react1,Espece *react2,Espece *prod1,Espece *prod2,double constante)
:cste(constante),reactif1(react1),reactif2(react2),produit1(prod1),produit2(prod2)
{
}

   Recepteur::Recepteur(Protein *prots,Protein *protc,Protein *protp,double c, double eq)
...
{
   if (this->composants.size()!=prot->composants.size()) return false;
   vector<Composant *>::iterator icomp1,icomp2;
   for (icomp1=this->composants.begin();icomp1!=this->composants.end();icomp1++){
      for (icomp2=prot->composants.begin();icomp2!=prot->composants.end();icomp2++){
	 if ((*icomp1)->protein==(*icomp2)->protein && (*icomp1)->nb==(*icomp2)->nb) break;
      }
      if (icomp2==prot->composants.end()) return false;
   }
   return true;
}
...
Protein *Protein::copyprot()
{
   ...
   for (icomp=composants.begin();icomp!=composants.end();icomp++){
      prot->composants.push_back(new Composant(static_cast<Protein *>((*icomp)->protein->copie),(*icomp)->nb));
   }
   if (phosphosite) prot->phosphosite=new Phosphosite();
...
{
   for (vector<Protein *>::iterator iprot=cell->proteins.begin();iprot!=cell->proteins.end();iprot++){
      if (this==*iprot){
         if (static_cast<Protein *>(this)->contain(prot)) return true;
      }
   }
   return false;
}
...
void Protein::addcomposant(Protein *prot)
{
   ...
   if (prot->composants.empty()){
      ...
      if (icompf==composants.end()) composants.push_back(new Composant(prot));
   }
   else{
      for(icompi=prot->composants.begin();icompi!=prot->composants.end();icompi++){
         for (icompf=composants.begin();icompf!=composants.end();icompf++){
         ...
         if (icompf==composants.end()) composants.push_back(new Composant((*icompi)->protein,(*icompi)->nb));
      }
   }
}
...
{//Modifie le taux de degradation d'une proteine quelconque
   ...
   cste*=c;
   if (cste<seuil_degrad_inf) cste=seuil_degrad_inf;
   if (cste>seuil_degrad_sup) cste=seuil_degrad_sup;
}

void Promoter::modifeq(double ampli)
{
   eqcte*=pow(ampli,2*frand()-1);
   if (eqcte<seuil_degrad_inf) eqcte=seuil_degrad_inf;
   if (eqcte>seuil_degrad_sup) eqcte=seuil_degrad_sup;
}

void Promoter::modiftrans(double ampli)
{
   transrate*=pow(ampli,2*frand()-1);
   if (transrate<seuil_degrad_inf) transrate=seuil_degrad_inf;
   if (transrate>seuil_degrad_sup) transrate=seuil_degrad_sup;
}
...
{//Modifie le taux de degradation d'un ARN quelconque
   double c=pow(ampli,2*frand()-1);
   cste*=c;
   if (cste<seuil_react_inf) cste=seuil_react_inf;
   if (cste>seuil_react_sup) cste=seuil_react_sup;
}
...
void Recepteur::modifcinetique(double ampli)
{
   ...
   if (recept_satur){
      if (modif<1.5){
         ...
         a0*=c;
         if (a0<seuil_degrad_inf) a0=seuil_degrad_inf;
         if (a0>seuil_degrad_sup) a0=seuil_degrad_sup;
      }
   } else {
      cste*=c;
      if (cste<seuil_degrad_inf) cste=seuil_degrad_inf;
...
void Protein::modifqtite(double ampli)
{
   double c=pow(ampli,2*frand()-1);
   qtite*=c;
   if (qtite<seuil_qtite_inf) qtite=seuil_qtite_inf;
   if (qtite>seuil_qtite_sup) qtite=seuil_qtite_sup;
}
...
void Protein::clivage(Protein &prot1,Protein &prot2)
{
   ...
   for (icomp=composants.begin();icomp!=composants.end();icomp++){
	 ...
	 prot1.composants.push_back(new Composant((*icomp)->protein,i));
      ...
      if (size-i){
	 prot2.composants.push_back(new Composant((*icomp)->protein,size-i));
      }
   }
}
...
Protein *Protein::addphospho()
{
   ...
   for (icomp=composants.begin();icomp!=composants.end();icomp++){
      prot->composants.push_back(new Composant((*icomp)->protein,(*icomp)->nb));
   }
   if (composants.empty()) prot->composants.push_back(new Composant(this,1));
