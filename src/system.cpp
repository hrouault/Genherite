...
void System::addrandrecept()
{
    ...
    if (anchorsignal) {
        ...
        while (prots == anchorprot) {
            prots =
                cellule->proteins[(int) (cellule->proteins.size() * frand())];
        }
...
void System::evolution()
{
    ...
    while (j) {
        ...
        case 0:
            //Dimérisation
            if (cellule->reactions.size() < nb_reactions_max
                && cellule->proteins.size() < nb_proteins_max) {
        ...
        case 1:
            //Modif réaction
            cellule->reactions[(int) (frand() * cellule->reactions.size())]->
                modifcinetique();
            j = 0;
            break;
        case 2:
            //Modif dégradation
            if (frand() < 0.8)
                cellule->proteins[(int) (frand() * cellule->proteins.size())]->
                    modifdegrad();
            else
                cellule->arns[(int) (frand() * cellule->arns.size())]->
                    modifdegrad();
            j = 0;
            break;
        case 3:
        ...
        case 4:
            ...
            if (cellule->reactions.size() < nb_reactions_max - 2
                && cellule->promoters.size() < nb_promoters_max) {
                Gene *randgene =
                    cellule->genes[(int) (frand() * cellule->genes.size())];
                Protein *anchorprot = cellule->proteins[nb_genes];
                Protein *randprot;
                if (anchorsignal) {
                    randprot =
                        cellule->
                        proteins[(int) (frand() * cellule->proteins.size())];
                } else {
                    randprot = anchorprot;
        ...
        case 5:
            ...
            if (cellule->reactions.size() < nb_reactions_max
                && cellule->proteins.size() < nb_proteins_max - 1) {
                cellule->addclivage();
                j = 0;
            }
            break;
            ...
            if (cellule->promoters.size()) {
                if (frand() < 0.5) {
                    cellule->
                        promoters[(int)
                                  (frand() *
                                   cellule->promoters.size())]->modifeq();
                } else {
                    if (frand() < 0.5) {
                        cellule->
                            promoters[(int)
                                      (frand() *
                                       cellule->promoters.size())]->
                            modiftrans();
                    } else
                        cellule->
                            genes[(int) (frand() * cellule->genes.size())]->
                            modiftrans();
                }
                j = 0;
            }
        ...
        case 12:
            ...
            if (recepteurs.size()) {
                recepteurs[(int) (frand() * recepteurs.size())]->
                    modifcinetique();
                j = 0;
            }
            break;
...
void System::printinternpart(ostream & out)
{
    ...
    for (irecept = recepteurs.begin(); irecept != recepteurs.end(); irecept++) {
        out << "\"" << *irecept << "\" [label=\"" << setprecision(2) <<
            (*irecept)->cste << "\"];";
        out << "\"" << (*irecept)->
            protsource << "\" -> \"" << *irecept <<
        ...
        out << "\"" << *irecept << "\" -> \"" << (*irecept)->
            protphospho << "\" [style=dashed,color=blue]; ";
    }
    out << "\n";
}
...
void System::printproperties(ostream & out)
{
    ...
    for (ivrecept ivr = recepteurs.begin(); ivr != recepteurs.end(); ivr++) {
        out << (*ivr)->protsource->label << "\t" << (*ivr)->protcible->
            label << "\t" << (*ivr)->cste << "\\n";
    }
    out << "\\n\"" << endl;
    out << "}" << endl;
...
double System::calculscoresystem3c()
{
    ...
    if (!flag) {
        ...
        for (unsigned int i = nb_steps; i < 2 * nb_steps; i++) {        //nb_steps;i<2*nb_steps;i++){
            ...
            score +=
                mini(dA12, dB12) + dinvA34 + dinvB34 + mini(dA13,
                                                            dB13) + mini(dA23,
                                                                         dB23);
        }
        for (unsigned int i = nb_steps / 2; i < nb_steps; i++) {        //nb_steps;i<2*nb_steps;i++){
            score +=
                0.5 * fabs(res.result1a[nb_steps + i] -
                           res.result1a[3 * nb_steps + i]);
            score +=
                0.5 * fabs(res.result2a[nb_steps + i] -
                           res.result2a[3 * nb_steps + i]);
            score +=
                0.5 * fabs(res.result3a[nb_steps + i] -
                           res.result3a[3 * nb_steps + i]);
            score +=
                0.5 * fabs(res.result1b[nb_steps + i] -
                           res.result1b[3 * nb_steps + i]);
            score +=
                0.5 * fabs(res.result2b[nb_steps + i] -
                           res.result2b[3 * nb_steps + i]);
            score +=
                0.5 * fabs(res.result3b[nb_steps + i] -
                           res.result3b[3 * nb_steps + i]);
        }
    } else {
        score = 1e10;
    }
    ...
    if (isnan(score))
        cout << "NaA observed!!" << endl;
    if (print_mode) {
        double dA12 =
            distcust(res.result1a[2 * nb_steps - 1],
                     res.result2a[2 * nb_steps - 1]);
        double dB12 =
            distcust(res.result1b[2 * nb_steps - 1],
                     res.result2b[2 * nb_steps - 1]);
        double dA13 =
            distcust(res.result1a[2 * nb_steps - 1],
                     res.result3a[2 * nb_steps - 1]);
        double dB13 =
            distcust(res.result1b[2 * nb_steps - 1],
                     res.result3b[2 * nb_steps - 1]);
        double dA23 =
            distcust(res.result2a[2 * nb_steps - 1],
                     res.result3a[2 * nb_steps - 1]);
        double dB23 =
            distcust(res.result2b[2 * nb_steps - 1],
                     res.result3b[2 * nb_steps - 1]);
        double dinvA34 =
            1 - distcust(res.result3a[2 * nb_steps - 1],
                         res.result4a[2 * nb_steps - 1]);
        double dinvB34 =
            1 - distcust(res.result3b[2 * nb_steps - 1],
                         res.result4b[2 * nb_steps - 1]);
        score =
            mini(dA12, dB12) + dinvA34 + dinvB34 + mini(dA13,
                                                        dB13) + mini(dA23,
                                                                     dB23);
...
double System::calculscoresystem2c()
{
    ...
    if (print_mode) {
        return fabs(res.result1a[2 * nb_steps - 1] -
                    res.result2a[2 * nb_steps - 1]);
    }
    return score;
}
...
int System::essential_recept()
{
    ...
    print_mode = false;
    //   print_mode=true;
    if (fabs(res.result1a[2 * nb_steps - 1] - res.result2a[2 * nb_steps - 1]) >
        0.2)
        return 1;
    //   print_mode=false;
    return 0;
...
System *System::copysystem()
{
    ...
    for (irecept = recepteurs.begin(); irecept != recepteurs.end(); irecept++) {
        Protein *prots =
            static_cast < Protein * >((*irecept)->protsource->copie);
        Protein *protc =
            static_cast < Protein * >((*irecept)->protcible->copie);
        Protein *protp =
            static_cast < Protein * >((*irecept)->protphospho->copie);
...
bool System::rmrandprot()
{
    ...
    for (ireact = cellule->reactions.begin();
         ireact != cellule->reactions.end(); ireact++) {
        if ((*ireact)->reactif1 == prot || (*ireact)->reactif2 == prot)
            return false;
    }
    for (vector < Promoter * >::iterator ipromo = cellule->promoters.begin();
         ipromo != cellule->promoters.end(); ipromo++) {
        if ((*ipromo)->protein == prot)
            return false;
    }
    vector < Recepteur * >::iterator irecept, delrecept;
    for (irecept = recepteurs.begin(); irecept != recepteurs.end(); irecept++) {
        if ((*irecept)->protsource == prot || (*irecept)->protcible == prot)
            return false;
    }
    for (ireact = cellule->reactions.begin();
         ireact != cellule->reactions.end(); ireact++) {
...
void System::optievolution()
{
    const double sommetaux =
        ...
        t_mod_recept_opti + t_mod_qpromo;
    const double taux[] =
        { t_mod_r_opti, t_mod_pro_opti, t_mod_qtite_opti, t_rmreact_opti,
t_mod_recept_opti, t_mod_qpromo };
    /* Sélection de la modification */
...
System *System::optimisation()
{
    ...
    for (isys = milieu.systems.begin(); isys != milieu.systems.end(); isys++)
        delete *isys;
