...
Protein *Cellule::searchcopyprot(Protein *prot)
{
   ...
   for (iprot=proteins.begin();iprot!=proteins.end();iprot++){
      if ((static_cast<Protein *>(*iprot)->copie)==prot) return *iprot;
   }
   return NULL;
}
...
void Cellule::copygene(Gene *gene)
{
   ...
   for (iprot=proteins.begin();iprot!=proteins.end();iprot++){
      for (icomp=(*iprot)->composants.begin();icomp!=(*iprot)->composants.end();icomp++){
   ...
   for (iprot=tempprotvec.begin();iprot!=tempprotvec.end();iprot++){
      Protein *prot=*iprot;
      for (string::iterator istr=prot->label.begin();istr!=prot->label.end();istr++){
   ...
   for (ipromo=temppromovec.begin();ipromo!=temppromovec.end();ipromo++){
      addpromotion((*ipromo)->gene,static_cast<Protein *>((*ipromo)->protein->copie),(*ipromo)->eqcte,(*ipromo)->transrate);
   }
   ...
   for (ipromo=temppromovec2.begin();ipromo!=temppromovec2.end();ipromo++){
      addpromotion(cgene,static_cast<Protein *>((*ipromo)->protein),(*ipromo)->eqcte,(*ipromo)->transrate );
   }
...
Cellule *Cellule::copycellule()
{
   ...
   for (ireact=reactions.begin();ireact!=reactions.end();ireact++){
      if ((*ireact)->copie==0){
	 react=*ireact;
	 (react->reactif1)?react1=react->reactif1->copie:react1=NULL;
	 (react->reactif2)?react2=react->reactif2->copie:react2=NULL;
	 (react->produit1)?prod1=react->produit1->copie:prod1=NULL;
	 (react->produit2)?prod2=react->produit2->copie:prod2=NULL;
	 Reaction *creact=cell->addreaction(react1,react2,prod1,prod2,(*ireact)->cste);
	 creact->copie=react;
	 react->copie=creact;
      }
   }
...
int Cellule::especenum(Espece *pesp)
{
   ...
   int i=1;
   for (iarn=arns.begin();iarn!=arns.end();iarn++){
      if (pesp==*iarn) return i;
      i++;
   }
   i=1+narns;
   for (iprot=proteins.begin();iprot!=proteins.end();iprot++){
      if (pesp==*iprot) return i;
      i++;
   }
...
Reaction *Cellule::adddimere(Protein *prot1,Protein *prot2,double c,double q,double cdimerisation)
{
   vector<Composant *>::iterator icomp;
   ...
   for (icomp=prot->composants.begin();icomp!=prot->composants.end();icomp++){
      for (int i=0;i<(*icomp)->nb;i++) prot->label += (*icomp)->protein->label;
   }
...
Reaction *Cellule::addreaction(Espece *react1,Espece *react2,Espece *prod1,
      Espece *prod2,double q)
{
...
Promoter *Cellule::addpromotion(Gene *gene,Protein *prot,double e,double t)
{
...
bool Cellule::addclivage()
{
   ...
   protini=proteins[i];
   for (icomp=protini->composants.begin();icomp!=protini->composants.end();icomp++){
      size+=(*icomp)->nb;
   }
   if (size<2){
      delete prot1;
...
{
   if (promoters.size()==0) return false;
   Promoter *promo;
...
bool Cellule::rmrandprot()
{
   ...
   for (vector<Promoter *>::iterator ipromo=promoters.begin();ipromo!=promoters.end();ipromo++){
...
{
   const double sommetaux=t_mod_r+t_mod_pro+t_mod_qtite+t_rmreact+t_mod_qpromo;
   const double taux[]={t_mod_r,t_mod_pro,t_mod_qtite,t_rmreact,t_mod_qpromo};
   ...
   switch(i){
	 ...
	 break;
      case 1:
	 //Modif dégradation
	 if (frand()<0.8) proteins[(int)(frand()*proteins.size())]->modifdegrad();
	 else arns[(int)(frand()*arns.size())]->modifdegrad();
	 return true;
	 break;
      case 2:
	 //Modif quantités
	 if (frand()<0.2) arns[(int)(frand()*arns.size())]->modifqtite();
	 else proteins[(int)(frand()*proteins.size())]->modifqtite();
	 return true;
	 break;
      case 3:
	 //Suppression de réactions
	 if (frand()<0.5){
	    if (rmrandpromo()) return true;
	 }
	 else{
	    if (rmrandprot()) return true;
	 }
	 break;
      case 4:
...
{ //procedure d'evolution
   ...
   switch(i){
      case 0:
	 //Dimérisation
	 if (reactions.size()<nb_reactions_max && proteins.size()<nb_proteins_max){
	    addrandreact();
	    return true;
	 }
	 break;
	 ...
	 if (reactions.size()<nb_reactions_max-2 && promoters.size()<nb_promoters_max){
	 ...
	 if (reactions.size()<nb_reactions_max && proteins.size()<nb_proteins_max-1){
	    addclivage();
	    return true;
	 }
	 break;
...
{ //procedure d'evolution
   const double sommetaux=t_new_r+t_mod_r+t_mod_pro+t_mod_qtite
      +t_new_promo+t_clivage+t_rmreact+t_phospho+t_mod_qpromo+t_dupgene+t_mutdir;
         ...
         case 8:
            ...
            if (promoters.size()){
               if (frand()<0.5){
                  ...
                  if (frand()<0.5){
                     promoters[(int)(frand()*promoters.size())]->modiftrans();
                  } else genes[(int)(frand()*genes.size())]->modiftrans();
               }
               j=0;
            }
            break;
...
{
   const double sommetaux=t_mod_r_opti+t_mod_pro_opti+t_mod_qtite_opti+t_rmreact_opti;
   const double taux[]={t_mod_r_opti,t_mod_pro_opti,t_mod_qtite_opti,t_rmreact_opti};
   ...
   while(j){
	 ...
	 case 0:
	    //Modif réaction
	    reactions[(int)(frand()*reactions.size())]->modifcinetique(ampli_opti);
	    j=0;
	    break;
	 case 1:
	    //Modif dégradation
	    if (frand()<0.5){
	       proteins[(int)(frand()*proteins.size())]->modifdegrad(ampli_opti);
	    }
	    else{
	 ...
	 case 2:
	    ...
	    if (frand()<0.3){
	       ...
	       proteins[(int)(frand()*proteins.size())]->modifqtite(ampli_opti);
	    }
	    j=0;
	    break;
	 case 3:
...
void Cellule::printinternpart(ostream &out)
{
   ...
   for (iarn=arns.begin();iarn!=arns.end();iarn++){
      out << "\"" << *iarn << "\" [label=\"" << (*iarn)->label << "\\nρ=" << setprecision(2) << (*iarn)->gene->transrate << ", δ=" << (*iarn)->cste << ", α=" << (*iarn)->translation->cste << "\"]; ";
   }
   out << "}" << endl;

   out << "{" << endl;
   for (iprot=proteins.begin();iprot!=proteins.end();iprot++){
      out << "\"" << *iprot << "\" [label=\"" << (*iprot)->label << ", δ=" << setprecision(2) << (*iprot)->cste << "\"]; ";
   }
   out << "}" << endl;
   ...
   for (ipromo=promoters.begin();ipromo!=promoters.end();ipromo++){
         out << "\"" << (*ipromo)->protein << "\" -> \"" << (*ipromo)->gene->arn << "\" [color=red, decorate=true, label=\"K=" << setprecision(2) << (*ipromo)->eqcte << "\\nρ=" << (*ipromo)->transrate << "\"";
   ...
   for (ireact=reactions.begin();ireact!=reactions.end();ireact++) (*ireact)->copie=NULL;
   for (igene=genes.begin();igene!=genes.end();igene++){
      (*igene)->arn->translation->copie=(*igene)->arn->translation;
   }
   for (ireact=reactions.begin();ireact!=reactions.end();ireact++){
      if ((*ireact)->reactif1 && (*ireact)->reactif2 && (*ireact)->produit1 && (*ireact)->produit2 &&
            (*ireact)->reactif1==(*ireact)->produit1){
         out << "\"" << *ireact << "\" [label=\"" << setprecision(2) << (*ireact)->cste << "\"];";
         out << "\"" << (*ireact)->reactif2 << "\" -> \"" << *ireact << "\"; ";
         out << "\"" << *ireact << "\" -> \"" << (*ireact)->produit2 << "\"; ";
         out << "\"" << (*ireact)->reactif1 << "\" -> \"" << *ireact << "\" [style=dashed]; ";
         (*ireact)->copie=*ireact;
      }
   }
   ...
   for (ireact=reactions.begin();ireact!=reactions.end();ireact++){
      if ((*ireact)->copie==NULL){
         out << "\"" << *ireact << "\" [label=\"" << setprecision(2) << (*ireact)->cste << "\"];";
         if ((*ireact)->reactif1){
            out << "\"" << (*ireact)->reactif1 << "\" -> \"" << *ireact << "\"; ";
         }
         if ((*ireact)->reactif2){
            out << "\"" << (*ireact)->reactif2 << "\" -> \"" << *ireact << "\"; ";
         }
         if ((*ireact)->produit1){
            out << "\"" << *ireact << "\" -> \"" << (*ireact)->produit1 << "\"; ";
         }
         if ((*ireact)->produit2){
            out << "\"" << *ireact << "\" -> \"" << (*ireact)->produit2 << "\"; ";
         }
...
void Cellule::printproperties(ostream &out)
{
   ...
   for (ivarn iva=arns.begin();iva!=arns.end();iva++){
      out << (*iva)->label << "\t" << (*iva)->qtite << "\t" << (*iva)->cste << "\\n";
   }
   out << "\\n";

   out << "Protein/Complex name\tinitial quantity\tdecay rate" << "\\n";
   for (ivprotein ivprot=proteins.begin();ivprot!=proteins.end();ivprot++){
      out << (*ivprot)->label << "\t" << (*ivprot)->qtite << "\t" << (*ivprot)->cste << "\\n";
   }
   out << "\\n";

   out << "Regulated gene Regulator\tEq const.\tTranscription rate" << "\\n";
   for (ivpromoter ivpromo=promoters.begin();ivpromo!=promoters.end();ivpromo++){
      out << (*ivpromo)->protein->label << "\t" << (*ivpromo)->gene->label << "\t" << (*ivpromo)->eqcte << "\t" << (*ivpromo)->transrate << "\\n";
   }
   out << "\\n";
   ...
   for (ivreaction ivr=reactions.begin();ivr!=reactions.end();ivr++){
      ...
      if ((*ivr)->reactif2) labr2=(*ivr)->reactif2->label;
      else labr2="";
      labp1=(*ivr)->produit1->label;
      if ((*ivr)->produit2) labp2=(*ivr)->produit2->label;
      else labp2="";
      out << labr1 << "\t" << labr2 << "\t->\t" << labp1 << "\t" << labp2 << "\t" << (*ivr)->cste << "\\n";
   }
...
//         resultat=ceff.integr(especenum(proteins[0]));
         break;
      case 3:
         resultat=ceff.integrporte(especenum(proteins[0]), especenum(proteins[args_info.nb_genes_arg]),
               especenum(proteins[args_info.nb_genes_arg+1]),false,false,false,false);
         break;
   }
...
//   }
   for (iprot=proteins.begin();iprot!=proteins.end();iprot++){
      out << "prot " << (*iprot)->label << " " << (*iprot)->qtite << " " << (*iprot)->cste << " ";
      if ((*iprot)->phosphosite) out << " 1 ";
      out << endl;
   }
//   for (ipromo=promoters.begin();ipromo!=promoters.end();ipromo++){
...
//   }
   for (ireact=reactions.begin();ireact!=reactions.end();ireact++){
      ...
      out << especenum((*ireact)->reactif1) << " " << especenum((*ireact)->reactif2) << " ";
      out << especenum((*ireact)->produit1) << " " << especenum((*ireact)->produit2) << endl;
   }
}
...
void Cellule::printcelluleintegr(ostream &out)
{
   ...
   for (iprot=proteins.begin();iprot!=proteins.end();iprot++){
      ...
      if ((*iprot)->phosphosite) out << "prot->phosphosite=new Phosphosite();\n";
      for (icomp=(*iprot)->composants.begin();icomp!=(*iprot)->composants.end();icomp++){
	 out << "prot->composants.push_back(new Composant(" << especestring((*icomp)->protein);
	 out << "," << (*icomp)->nb << "));\n";
      }
   }
   out << "Promoter *promo;\n";
   ...
   for (ireact=reactions.begin();ireact!=reactions.end();ireact++){
      ...
      out << especestring((*ireact)->reactif1) << "," << especestring((*ireact)->reactif2) << ",";
      out << especestring((*ireact)->produit1) << "," << especestring((*ireact)->produit2);
      out << "," << (*ireact)->cste << ");\n";
      out << "cell->reactions.push_back(react);\n";
   }
   int i=0;
   for (igene=genes.begin();igene!=genes.end();igene++){
      out << "cell->genes[" << i << "]->arn=" << especestring((*igene)->arn) << ";\n";
      i++;
   }
   i=0;
   for (iarn=arns.begin();iarn!=arns.end();iarn++){
      out << "cell->arns[" << i << "]->protein=" << especestring((*iarn)->protein) << ";\n";
      out << "cell->arns[" << i << "]->translation=" << reactionstring((*iarn)->translation) << ";\n";
      i++;
   }
   i=0;
   for (ipromo=promoters.begin();ipromo!=promoters.end();ipromo++){
...
double Cellule::scorefunction(vector<double> fonction,vector<double> resultatintegr)
{
  double s=0;
  vector<double>::iterator idbres,idbf;
  
  for (idbres=resultatintegr.begin();idbres!=resultatintegr.end();idbres++){
